 ...
 *
 * Disassembly of SSDT6.aml, Sat Jul 13 00:07:39 2019
 *
 * Original Table Header:
 *     Signature        "SSDT"
 *     Length           0x00001F06 (7942)
 *     Revision         0x01
 *     Checksum         0x6C
 *     OEM ID           "OptRef"
...
DefinitionBlock ("", "SSDT", 1, "OptRef", "OptTabl", 0x00001000)
{
    ...
    External (_SB_.PCI0.LPCB.EC0_.XHPP, UnknownObj)    // (from opcode)
    ...
    Scope (\_SB.PCI0.RP05.PEGP)
    {
        Method (GC6I, 0, Serialized)
        {
            ...
            While (LNotEqual (\_SB.GGIV (0x04010013), One))
            {
                Sleep (One)
            }
        }
        ...
        Method (GC6O, 0, Serialized)
        {
            ...
            If (LEqual (\_SB.GGIV (0x04010013), One))
            {
                \_SB.PCI0.SGPO (Zero, 0x04010015, Zero, One)
                While (LNotEqual (\_SB.GGIV (0x04010013), Zero))
                {
                    Sleep (One)
                }

                \_SB.PCI0.SGPO (Zero, 0x04010015, Zero, Zero)
            }
        ...
        {
            If (LEqual (\_SB.GGIV (0x04010013), Zero))
            {
                    ...
                    Switch (PSH0)
                    {
                        Case (Zero)
                        {
                            If (CPUT)
                            {
                                Store (0x0200, RETN)
                                Or (RETN, PSH0, RETN)
                                Store (\_SB.PCI0.LPCB.EC0.XHPP, PDTS)
                            }
